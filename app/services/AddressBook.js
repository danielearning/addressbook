...
service.factory('AddressBook', [
  '$window', // Since localStorage belongs to window.
  ...
  function($window, $emailvalidator, $countryvalidator, $cypher){
    ...
    function loadAddressBook() {
      ...
      var decrypt = $cypher && $cypher.available() ? $cypher.decrypt : x => x;
    ...
    function saveAddressBook(book) {
      var encrypt = $cypher && $cypher.available() ? $cypher.encrypt : x => x;
     ...
     * */
    function _remove(params, callback) { // The delete
    ...
    return {
      get: function(params, callback) { // Basic entry get.
        enforcePhoneBook();
      ...
      validate: function(entry) { // Validate only, like in Sequelize
        return validateEntry(entry);
      },
      
      save: function(entry, callback) { // Validate and save entry
        enforcePhoneBook();
      ...
      query: function(callback) { // A get all
        enforcePhoneBook();
        var r = [];
        var book = loadAddressBook();
        for (var i in book) {
